...
export default function AbsensiPage() {
  ...
  return (
    <>
      ...
      {/* 12 tabel bulanan — data dari attendanceMap */}
              ...
              <tbody>
                {list.map((siswa, sIdx) => {
                  ...
                  for (let d = 1; d <= mConfig.days; d++) {
                    // index siswa + tanggal + index bulan → attendanceMap
    ...
    <>
      ...
      <div className="grid-4">
        {[
          ...
          <div key={item.label} className="glass-card text-center" style={{ padding: "8px" }}>
            <div className="title-sub" style={{ fontSize: "7.5px" }}>{item.label}</div>
            <div className="card-val" style={{ color: item.color, fontSize: "13px" }}>
              {((item.val / n) * 100).toFixed(1)}%
            </div>
          </div>
        ))}
              ...
              {list.map((s, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td style={{ color: "#4ade80", fontWeight: 800 }}>{s.hadir}</td>
                    <td style={{ color: "#60a5fa", fontWeight: 800 }}>{s.izin}</td>
                    <td style={{ color: "#facc15", fontWeight: 800 }}>{s.sakit}</td>
                    <td style={{ color: "#f43f5e", fontWeight: 800 }}>{s.alpa}</td>
                    <td>
                      <div className="pct-breakdown">
                        <span style={{ color: "#4ade80" }}>H:{pctH}%</span>
                        <span style={{ color: "#60a5fa" }}>I:{pctI}%</span>
      ...
      <div className="section-divider">
        <div className="divider-badge">
          <i className="fa-solid fa-calendar-days" style={{ marginRight: "5px" }} />
          Rekap Harian Per Bulan (Juli 2026 - Juni 2027)
        </div>
      </div>
      ...
      {/* 12 BULAN PENUH */}
      {monthConfigs.map((mConfig, mIdx) => (
        <div key={mConfig.name} className="glass-card" style={{ padding: "10px" }}>
          <div
            className="flex-between"
            style={{
              marginBottom: "10px",
